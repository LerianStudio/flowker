...
# Script to verify API documentation consistency for flowker
...
echo -e "${CYAN}--------------------------------------------------------------------${NC}"
echo -e "${CYAN}   Verifying API documentation for flowker        ${NC}"
echo -e "${CYAN}--------------------------------------------------------------------${NC}"
...
ensure_dependencies() {
  if [ -d "$COMPONENT_DIR/scripts" ]; then
    if [ ! -d "$COMPONENT_DIR/scripts/node_modules/js-yaml" ]; then
      echo -e "${YELLOW}Installing dependencies in flowker/scripts...${NC}"
