...
func newTestExecutorConfiguration(t *testing.T, createdAt, updatedAt time.Time) *model.ExecutorConfiguration {
	t.Helper()

	endpoint := model.NewExecutorEndpointFromDB("execute", "/execute", "POST", 30)
	auth := model.NewExecutorAuthenticationFromDB("none", nil)
...
func TestGetSortValue_CreatedAt(t *testing.T) {
	...
	ec := newTestExecutorConfiguration(t, createdAt, updatedAt)

	result := getSortValue(ec, "createdAt")
...
func TestGetSortValue_UpdatedAt(t *testing.T) {
	...
	result := getSortValue(ec, "updatedAt")
...
func TestGetSortValue_Name(t *testing.T) {
	...
	result := getSortValue(ec, "name")

	assert.Equal(t, "test-executor", result)
}
...
func TestGetSortValue_Default(t *testing.T) {
	...
	result := getSortValue(ec, "something_unknown")

	// Default falls back to createdAt
	expected := "2025-06-15T10:00:00.000Z"
	assert.Equal(t, expected, result)
}
